...
class http_coon {
  public:
    enum HTTP_CODE {
        NO_REQUESTION,  // 代表请求不完整，需要客户继续输入
        GET_REQUESTION, // 代表获得并且解析了一个正确的HTTP请求
        BAD_REQUESTION, // 代表HTTP请求语法不正确
        FORBIDDEN_REQUESTION, // 代表访问资源的权限有问题
  ...
  private:
    ...
    int jude_line(int &check_index,
                  int &read_buf_len);   // 该请求是否是完整的以行\r\n
...
// read函数的封装= read_buf + (strlen(read_buf) - content_length);
    // argv[strlen(argv) + 1] = '\0';
int http_coon::myread() {
    bzero(&read_buf, sizeof(read_buf));
    while (true) {
        int ret =
            recv(client_fd, read_buf + read_count, READ_BUF - read_count, 0);
        if (ret == -1) {
...
bool http_coon::succeessful_respond() {
    ...
    sprintf(requst_head_buf,
            "HTTP/1.1 200 ok\r\nConnection: close\r\ncontent-length:%d\r\n\r\n",
            file_size);
}

// 400
bool http_coon::bad_respond() {
    ...
    bzero(requst_head_buf, sizeof(requst_head_buf));
    sprintf(requst_head_buf,
            "HTTP/1.1 400 BAD_REQUESTION\r\nConnection: "
            "close\r\ncontent-length:%d\r\n\r\n",
            file_size);
}

// 403
bool http_coon::forbiden_respond() {
    bzero(url, strlen(url));
